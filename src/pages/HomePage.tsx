...
const HomePage: React.FC = () => {
  ...
  const handleEnrichWord = useCallback(async (item: DictionaryItem) => {
    ...
    const payload: WordDetailsInput = {
      CEFR: item.cefr,
      type: item.wordType,
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/words/detail`, {
        method: 'POST',
        headers: {
      ...
      const result: WordDetailsResponse = await response.json();
      setSelectedWordData(result.data);
